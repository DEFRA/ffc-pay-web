...
const { schemeAdmin, holdAdmin, dataView, closureAdmin, statusReportSfi23, statusReportsDelinked, manualPaymentsAdmin } = require('../../../app/auth/permissions')
...
describe('dev authentication', () => {
  ...
  test('authenticate should set scopes in cookieAuth', async () => {
    await devAuth.authenticate('redirectCode', mockCookieAuth)
    expect(mockCookieAuth.set.mock.calls[0][0].scope).toStrictEqual([holdAdmin, schemeAdmin, dataView, closureAdmin, statusReportSfi23, statusReportsDelinked, manualPaymentsAdmin])
  })
  ...
  test('refresh should return roles', async () => {
    ...
    expect(result).toStrictEqual([holdAdmin, schemeAdmin, dataView, closureAdmin, statusReportSfi23, statusReportsDelinked, manualPaymentsAdmin])
  })
