...
const handleManualPaymentUploadPost = async (request, h) => {
  ...
  if (isTextWhitespace(fileContent)) {
    ...
    return manualPaymentUploadFailAction(
      request,
      h,
      Boom.boomify(new Error('File is empty'), { statusCode: UNPROCESSABLE_CONTENT })
    )
  }

  return processManualPaymentFile(request, h, {
    filePath,
    filename,
    uploaderNameOrEmail,
    jobId
...
async function processManualPaymentFile (request, h, { filePath, filename, uploaderNameOrEmail, jobId }) {
  ...
  try {
    ...
    const response = await postInjection(MANUAL_UPLOAD, {
      uploader: uploaderNameOrEmail,
      filename
    }, null)
    ...
    await finalizeStatus(request, jobId, {
      status,
      message,
      isSuccess,
      statusCode
  ...
  return h
    .redirect(MANUAL_PAYMENT_ROUTES.MANUAL_PAYMENTS)
    .state('manualPaymentUpload', isSuccess
      ? { success: true }
      : { success: false, errors: { details: [{ path: 'file', message: errorMessage }] } }
    )
}
